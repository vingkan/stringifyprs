...
async function fetchAndOutputPRs() {
  ...
  const allPullRequestData = await fetchPullRequestsInBatches(
    prInfos,
    accessToken,
    5
  );
  ...
  for (const fetchedData of allPullRequestData) {
    ...
    for (const file of filesData) {
      ...
      const inlineComments = reviewCommentsData
        .filter((c) => c.path === file.filename && c.diff_hunk)
        .sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
      const threads = {};
      inlineComments.forEach((comment) => {
        if (!comment.in_reply_to_id)
          threads[comment.id] = { comment, replies: [] };
        else if (threads[comment.in_reply_to_id])
          threads[comment.in_reply_to_id].replies.push(comment);
      });

      Object.values(threads).forEach(({ comment, replies }) => {
        result += `> Comment by ${comment.user.login} at ${comment.created_at}\n${comment.body}\n\n`;
        replies.forEach((reply) => {
          const replyIndented = reply.body
            .split("\n")
            .map((l) => "  " + l)
            .join("\n");
    ...
    const nonInlineComments = commentsData
      .filter((c) => !c.path && !c.diff_hunk)
      .sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
    const outerThreads = {};
    nonInlineComments.forEach((comment) => {
      if (!comment.in_reply_to_id)
        outerThreads[comment.id] = { comment, replies: [] };
      else if (outerThreads[comment.in_reply_to_id])
        outerThreads[comment.in_reply_to_id].replies.push(comment);
    });
  ...
  document.getElementById(
    "downloadBtn"
  ).textContent = `Download (${sizeMB} MB)`;
}
...
const fetchBtn = document.getElementById("fetchBtn");
fetchBtn.addEventListener("click", processPRs);
